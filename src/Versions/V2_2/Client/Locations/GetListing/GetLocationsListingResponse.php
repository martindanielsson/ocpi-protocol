...
namespace Chargemap\OCPI\Versions\V2_2\Client\Locations\GetListing;
...
class GetLocationsListingResponse extends BaseResponse
{
    ...
    public static function from(GetLocationsListingRequest $request, ResponseInterface $response): GetLocationsListingResponse
    {
        ...
        $json = self::toJson($response, 'V2_2/eMSP/Client/Locations/locationGetListingResponse.schema.json');

        $return = new self();
        foreach ($json->data ?? [] as $item) {
            if (PayloadValidation::isValidJson('V2_2/eMSP/Client/Locations/location.schema.json', $item)) {
